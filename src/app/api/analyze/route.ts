...
const JUDGMENT_CRITERIA = `★판정 기준 핵심 원칙★
- caution/danger는 의학적으로 임산부에게 실질적 위험이 있는 성분에만 부여해줘.
- 아래 성분들은 일반 가공식품에 보편적으로 사용되며 임산부에게 특별한 위험이 없으므로 반드시 "success"로 분류해줘:
  설탕, 포화지방, 나트륨, 단백질, 탄수화물, 유화제, 증점제, 산도조절제, 향료, 합성향료, 천연향료, 카라멜색소, 치자황색소 등 타르색소가 아닌 천연·합성착색료, 구연산, 젖산, 혼합분유, 탈지분유, 유청분말
  에리스리톨, 알룰로스, 스테비아, 말티톨 (FDA·식약처 승인 감미료, 임산부 적정 섭취 시 안전)
- caution을 줄 수 있는 성분: 카페인·과라나·마테(음료), 액상과당·고과당콘시럽(혈당), 아질산나트륨(가공육), 수크랄로스·아세설팜칼륨·아스파탐·사카린(인공감미료 — 태반 통과 가능, 과다 시 주의), 타르색소(적색2호·황색4호 등), 경화유·부분경화유(트랜스지방), 참치·연어·훈제연어(수은·리스테리아), 레티놀 고함량(비타민A), 감초추출물·글리시리진
...
const WEEK_ANALYSIS_GUIDE = (pregnancyWeeks: number | null) =>
  pregnancyWeeks !== null
    ? `현재 사용자는 임신 ${pregnancyWeeks}주차입니다.
description은 왜 주의/위험/안전한지 결론 한 줄로 작성해줘.
weekAnalysis는 위험 이유 재설명 없이 "${pregnancyWeeks}주차에 얼마나/어떻게/대신 무엇을" 기준으로 실천 가능한 행동 지침을 2문장 이내로 작성해줘.
★weekAnalysis 핵심 원칙★: "과다 섭취는 피하세요" 같은 막연한 표현 금지. 반드시 구체적인 수량/횟수 기준을 제시해줘.
  예시(인공감미료): "수크랄로스 ADI 기준으로 체중 60kg 기준 하루 500ml 캔 3개까지는 허용 범위예요. 매일보다는 가끔 한 캔 정도로 즐기는 걸 추천해요."
  예시(카페인): "임산부 하루 카페인 권고량은 200mg이에요. 이 제품 한 캔(카페인 OOmg)은 하루 권고량의 OO%로, 하루 1캔 이내로 드세요."
  예시(나트륨): "임신 중 나트륨 하루 권고량은 2,000mg이에요. 이 제품 1회 섭취 시 OOmg으로, 하루 OO번 이상은 피하세요."`
    : `일반적인 임산부 기준으로 weekAnalysis를 작성해줘.
★weekAnalysis 핵심 원칙★: "과다 섭취는 피하세요" 같은 막연한 표현 금지. 반드시 구체적인 수량/횟수 기준을 제시해줘.
  예시(인공감미료): "수크랄로스 ADI 기준으로 체중 60kg 기준 하루 500ml 캔 3개까지는 허용 범위예요. 매일보다는 가끔 한 캔 정도로 즐기는 걸 추천해요."
  예시(카페인): "임산부 하루 카페인 권고량은 200mg이에요. 하루 1캔 이내로 드세요."`;
...
${WEEK_ANALYSIS_GUIDE(pregnancyWeeks)}
...
다음 JSON 형식으로 응답해줘:
{
  ...
  "productName": "한국어 브랜드명(있으면) + 한국어 제품명을 자연스럽게 조합해서 반환해줘. 예: '매일 카페라떼 파르페', '빙그레 바나나맛우유', '칭따오 맥주 백맥'. 브랜드명이 없으면 제품명만. 외국 제품이면 원래 표기 유지. ★절대 금지★: 영문 병기·원재료명·용량·% 수치·괄호 안 성분 나열을 제품명에 포함하지 마. 패키지에 한국어와 영문이 함께 표기된 경우 한국어만 사용해줘. 단, 괄호 안에 한국어 번역이 있으면 그 한국어를 제품명으로 사용해줘 (예: 'HEYROO Strawberry Red Bean Ice flakes (딸기연유팥빙수)' → '헤이루 딸기연유팥빙수'). 언더스코어(_)는 공백으로 바꿔줘. 앞뒤 따옴표는 제거해줘.",
    ...
    if (barcode) {
      ...
      if (!product) {
        if (!imageBase64) {
          ...
          if (result.status === 'error_db_mismatch') {
            ...
            result.headline = result.headline || '데이터베이스에 없는 제품이에요';
            result.productName = result.productName || '알 수 없는 제품';
          } else {
            // 검색으로 찾은 경우 catalog에 저장 (다음 조회 시 캐시 활용)
            ...
            supabase.from('catalog').upsert({
              ...
              if (error) console.error('[analyze] catalog upsert failed (barcode-search):', error);
            });
          }
...
다음 JSON 형식으로 응답해줘:
{
  ...
  "productName": "한국어 브랜드명(있으면) + 한국어 제품명을 자연스럽게 조합해서 반환해줘. 예: '매일 카페라떼 파르페', '빙그레 바나나맛우유', '칭따오 맥주 백맥'. 브랜드명이 없으면 제품명만. 외국 제품이면 원래 표기 유지. ★절대 금지★: 영문 병기·원재료명·용량·% 수치·괄호 안 성분 나열을 제품명에 포함하지 마. 패키지에 한국어와 영문이 함께 표기된 경우 한국어만 사용해줘. 단, 괄호 안에 한국어 번역이 있으면 그 한국어를 제품명으로 사용해줘 (예: 'HEYROO Strawberry Red Bean Ice flakes (딸기연유팥빙수)' → '헤이루 딸기연유팥빙수'). 언더스코어(_)는 공백으로 바꿔줘. 앞뒤 따옴표는 제거해줘. 식별 불가시 '알 수 없음'.",
