...
import { Scan, ShieldCheck, Search, CheckCircle2, Calendar } from 'lucide-react';
...
export default function HomePageCal() {
  ...
  return (
    <div className="flex flex-col flex-1 bg-bg-canvas pb-20">
      ...
      {/* ── Top Utility Area: shadow 강화 ── */}
      <div className="px-4 pt-4 space-y-2">
        {!isLoggedIn ? (
          <div
            className="bg-primary/10 border border-primary/20 rounded-xl p-3 flex items-center justify-between cursor-pointer shadow-[0_1px_3px_rgba(0,0,0,0.06),0_4px_12px_rgba(0,0,0,0.04)]"
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <div className="text-left min-w-0">
                <p className="text-xs text-text-secondary leading-none mb-0.5">임신 주차</p>
            ...
            <button
              ...
              className="flex-1 flex items-center justify-between gap-2 bg-bg-surface border border-border-subtle rounded-xl px-3 py-2.5 hover:bg-neutral-bg transition-colors shadow-[0_1px_3px_rgba(0,0,0,0.06),0_4px_12px_rgba(0,0,0,0.04)]"
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="text-left min-w-0">
                  <p className="text-xs text-text-secondary leading-none mb-0.5">남은 스캔</p>
              ...
              {!isActive && (
                <span className="text-xs font-medium text-secondary bg-secondary/10 px-1.5 py-0.5 rounded-full shrink-0">충전</span>
              )}
            </button>
          </div>
        )}
        ...
        {hasPendingMonthly && (
          <div className="bg-caution-bg border border-caution-fg/20 rounded-xl p-3 flex items-center space-x-2 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
      ...
      {/* ── Hero: Cal.com 방식 shadow + 여백 확대 + letter-spacing ── */}
      <section className="px-4 pt-4 pb-6">
        <div className="bg-accent rounded-[28px] p-7 shadow-[0_2px_8px_rgba(0,0,0,0.08),0_8px_24px_rgba(0,0,0,0.06)] relative overflow-hidden">
          <div className="relative z-10 flex flex-col items-start">
            ...
            <p className="text-sm text-text-secondary mb-5 max-w-[240px] leading-relaxed">
              임산부 기준 성분 분석부터<br />안전한 대체 제품 추천까지 5초면 충분해요.
            </p>
            {/* Cal.com 스타일 버튼: rounded-xl (12px), font-bold */}
            <button
              className="w-full h-12 bg-primary text-white rounded-xl text-sm font-medium flex items-center justify-center gap-2 shadow-[0_1px_3px_rgba(0,0,0,0.1),0_4px_12px_rgba(242,140,130,0.3)] hover:bg-primary-strong transition-colors"
      ...
      {/* ── Features: gap 확대, shadow 선명화 ── */}
      <section className="px-4 py-4 space-y-5">
        <h2 className="text-[22px] font-bold text-text-primary px-1">
          마미스캔이 도와드릴게요
        </h2>
        <div className="grid gap-5">
          {[
            ...
            <div
              key={title}
              className="bg-bg-surface border border-border-subtle rounded-[20px] shadow-[0_1px_3px_rgba(0,0,0,0.08),0_4px_16px_rgba(0,0,0,0.06)] p-5 flex items-start space-x-4"
            >
      ...
      {/* ── Trust: 기존 구조, letter-spacing만 추가 ── */}
      <section className="px-4 py-6 mt-2 bg-white border-y border-border-subtle">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-text-secondary px-4 leading-relaxed">
